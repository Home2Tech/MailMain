...
export default function CampaignComposer({ templateId, subject: initialSubject, htmlBody, onClose }: CampaignComposerProps) {
  ...
  useEffect(() => {
    ...
    supabase.from("app_settings").select("resend_from_email").eq("id", true).single().then(({ data }) => { if (data?.resend_from_email) setFromEmail(data.resend_from_email); });
  }, []);
  ...
  async function createCampaign() {
    ...
    const { data, error: createError } = await supabase.from("email_campaigns").insert({ template_id: templateId, list_id: listId, subject: subject.trim(), html_body: htmlBody, from_email: fromEmail, status, scheduled_for: scheduled, recipient_count: recipientCount ?? 0 }).select().single();
