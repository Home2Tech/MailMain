...
export default function WorkflowManager() {
  ...
  function startNew() { setEditingId(null); setDraft({ ...emptyDraft, template_id: templates[0]?.id ?? "", list_id: lists[0]?.id ?? "" }); setError(null); }
  ...
  async function save() {
    if (!draft.name || !draft.trigger_event || !draft.template_id || !draft.list_id) { setError("Rule name, trigger, recipient list, and template are required."); return; }
    setSaving(true); setError(null);
    const result = editingId ? await supabase.from("automation_rules").update(draft).eq("id", editingId) : await supabase.from("automation_rules").insert(draft);
    setSaving(false);
